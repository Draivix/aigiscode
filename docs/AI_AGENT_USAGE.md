...
16. Run `aigiscode agent-run /repo --adapter codex-exec` when you want AigisCode to execute a real local Codex review and write `agent-review.json`, `agent-review.md`, `agent-output-schema.json`, and `agent-execution.jsonl`. The unvalidated adapter response is retained separately in `agent-review.raw.json`.
